// * Eshan Nahar
// Design and simulate any data structure like stack or queue visualization
// using graphics. Simulation should include all operations performed on
// designed data structure. Implement the same using OpenGL.
...
so we use a flag called keyboard callback to identify between various functions
* which will call the keyboard callback function

* To determine size of stack flag = 1
* To either push or pop from stack flag = 2
*/
int key1 = 3; // Variable `key1` set to 3 to display initial window
int SCREENH = 450, SCREENW = 450; // The screen size
int mouseval = 1;                 // A flag for mouse callback
...
char stk[] = "Stack", que[] = "Queue", ext[] = "EXIT", sk[] = "STACK",
     di_qu[] = "QUEUE";
...
void frntpge() {
  mouseval = -30;
  char by[] = "by", title[] = "IMPLEMENTATION OF DS",
       ta[] = "Tharif Ansari A           1RN16CS115";
  ...
  if(mouseval == 1) {
    switch(key1) {
      ...
      case 2:
        ...
        queue();
        break;
      case 3: frntpge(); break;
      case 70: welcome(); break;
      case 4: exit(0);
    }
  }
  glFlush();         // Render now
...
void mouse_callback_function(int button, int state, int ax,
                             int ay) // Takes input from mouse
{
  int mx = ax * 100 / SCREENW,
      my = (SCREENH - ay) * 100 /
           SCREENH; // Here, m: mouse coordinate to graphics
  ...
  if(down && mouseval == -30) {
    ...
    if(mx > (19.7) && mx < (80.3) && my > (20) && my < (25)) { // Exot
      glClear(GL_COLOR_BUFFER_BIT);
      key1 = 7;
      display1();
    }
    ...
    // Stack to home
    if(mx > (39.7) && mx < (60.3) && my > (5) && my < (10)) {
      glClear(GL_COLOR_BUFFER_BIT);
      key1 = 8;
      display_back();
    }
  } else if((down && mouseval == 3) || (down && mouseval == 4)) {
    if(mx > (39.7) && mx < (60.3) && my > (5) && my < (10) && mouseval == 3) {
      // Stack to home
      glClear(GL_COLOR_BUFFER_BIT);
      key1 = 8;
      display_back();
    } else if(mx > (69.7) && mx < (90.3) && my > (5) && my < (10) &&
              mouseval == 4) {
      // To stack size
      glClear(GL_COLOR_BUFFER_BIT);
      key1 = 9;
  ...
  glutMouseFunc(
      mouse_callback_function); // Register callback handler for mouse event
}

void displaying_stack() {
  bg_color();
  glColor3f(0, 0, 0);
  drawString(45, 95, 0, GLUT_BITMAP_HELVETICA_18,
             sk); // User definded function to write inisde the box
  ...
  if(stk_size < 5) {
    ...
    sx4 = 60;
    sy4 = 30;
    for(int z = 0; z < stk_size; z++) draw_rectangle();
  } else {
...
void displaying_simple_queue() {
  ...
  glColor3f(0, 0, 0);
  drawString(45, 95, 0, GLUT_BITMAP_HELVETICA_18,
             di_qu); // User definded function to write inisde the box
  ...
  if(que_size < 5) {
    ...
    sx3 = 30;
    sy3 = 60;
    for(int z = 0; z < que_size; z++) draw_rectangle();
  } else {
    glColor3f(0, 0, 0);
...
void welcome() {
  ...
  glRectf(40, 40, 60, 45);
  glColor3f(0.137, 0.137, 0.556);
  drawString(45, 41, 0, GLUT_BITMAP_HELVETICA_18,
             stk); // User definded function to write inisde the box
...
void pop() {
  if(stk_top > -1) {
    ...
    glRectf(19, 0, 39, 90);
    glColor3f(0, 0, 0); //
    drawString(20, stk_top_ptr_pos - 10, 0, GLUT_BITMAP_HELVETICA_18,
               top_pointer);
    glColor3f(0.3, 0.56, 0.84);
...
void enqueue(char key) {
  ...
  if(que_rear == que_size - 1) {
    ...
    drawString(que_front_elem_ptr_pos, 54, 0, GLUT_BITMAP_HELVETICA_18,
               key_ptr);
...
void dequeue() {
  if(que_frnt == -1 || que_frnt > que_rear) {
    ...
    glRectf(1, 39, 95, 49);
    glColor3f(0, 0, 0);
    drawString(que_front_ptr_pos + 10, 46, 0, GLUT_BITMAP_HELVETICA_18,
               front); //
    glColor3f(0.3, 0.56, 0.84);
...
int main(int argc, char **argv) {
  ...
  glutReshapeFunc(Reshape_callback_function);   // Register callback handler for
                                                // Window re-size event
  glutKeyboardFunc(keyboard_callback_function); // Register callback handler for
                                                // Special-key event
  glutMouseFunc(
      mouse_callback_function); // Register callback handler for mouse event
  glutDisplayFunc(display); // Invokes a function to create a picture within the
...
void Reshape_callback_function(int w, int h) {
  ...
  if(key1 == 3) { glOrtho(0.0, 100.0, 0.0, 100.0, -5.0, 10.0); }
}
