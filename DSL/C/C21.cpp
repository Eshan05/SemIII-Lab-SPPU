...
class Appointment {
	public:
		...
		Appointment(int sh, int sm, int eh, int em, string i)
				: startHour(sh), startMinute(sm), endHour(eh), endMinute(em), id(i), next(nullptr) {}
};
...
class AppointmentList {
	public:
		...
		void displayAppointments(int dayStartHour, int dayStartMinute, int dayEndHour, int dayEndMinute) {
		...
		void sortAppointmentsByTime() {
				...
				while (current) {
						...
						if (!sorted || compareAppointments(current, sorted) < 0) {
								...
								while (temp->next && compareAppointments(current, temp->next) >= 0) { temp = temp->next; }
