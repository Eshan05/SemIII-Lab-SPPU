...
class Student {
 ...
 public:
  Student(string n, int rn, string c, char d, string dob, string bg, string st,
          string city, string state, string country, int zip, long tel,
          string dl)
      : name(n), rollNumber(rn), className(c), division(d), dateOfBirth(dob),
        bloodGroup(bg), telephoneNumber(tel), drivingLicenseNumber(dl) {
  ...
  Student()
      : name("Unknown"), rollNumber(0), className("Unknown"), division('A'),
        dateOfBirth("01/01/1900"), bloodGroup("Unknown"), telephoneNumber(0),
  ...
  Student(const Student &other)
      : name(other.name), rollNumber(other.rollNumber),
        className(other.className), division(other.division),
  ...
  void setContactAddress(const string &st, const string &city,
    ...
    contactAddress =
        new Address(st, city, state, country, zip); // Allocate new memory
  }
};
...
int main() {
  ...
  do {
    ...
    switch(choice) {
      case 1: {
        string name, className, dateOfBirth, bloodGroup, street, city, state,
            country, drivingLicenseNumber;
        ...
        students[studentCount++] =
            new Student(name, rollNumber, className, division, dateOfBirth,
                        bloodGroup, street, city, state, country, zipCode,
                        telephoneNumber, drivingLicenseNumber);
